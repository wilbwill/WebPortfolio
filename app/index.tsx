...
export default function Index() {
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      function dottify() {
        if (ctx === null) return;
        let data = ctx.getImageData(TEXTBOX.x, TEXTBOX.y, TEXTBOX.w, TEXTBOX.h)
            .data,
          pixa = data
            .reduce(
              (
                a: { x: number; y: number; rgb: number[] }[],
                c,
                i,
                o
              ) => {
        ...
        pixa.forEach((c, i) => {
          PARTICLES[i] = new Particle(
            TEXTBOX.x + c.x,
            TEXTBOX.y + c.y,
            [c.rgb[0] || 0, c.rgb[1] || 0, c.rgb[2] || 0] as [
              number,
              number,
              number
            ]
      ...
      function write() {
        ...
        ctx.fillText(TEXTBOX.str, 0.5 * _C.width, 0.5 * _C.height);
